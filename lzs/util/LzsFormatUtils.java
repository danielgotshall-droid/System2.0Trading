...
public final class LzsFormatUtils {
  ...
  public static String buildHudSide(LzsSideState state, LzsConfig cfg, LzsContextResult context, LzsMergedContextResult merged, LzsContextConfig ctxCfg) {
    ...
    if (state.interaction.opposingZoneConflict && state.interaction.opposingZoneConflictTag != null && state.interaction.opposingZoneConflictTag.length() > 0 && mode != LzsHudDisplayMode.COMPACT) {
      sb.append("\nOppZone ").append(state.interaction.opposingZoneConflictTag);
    }
