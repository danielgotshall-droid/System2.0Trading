...
public class LzsTradeManagementEngine {
  ...
  public double getStructuralProtectReference(LzsSide side, long lookbackMs) {
    java.util.List<LzsTradeManagementSnapshot> items = primaryWindow.copyItems();
    if (items == null || items.size() < 2 || side == null) return Double.NaN;
    long nowMs = items.get(items.size() - 1).nowMs;
    java.util.List<LzsTradeManagementSnapshot> recent = new java.util.ArrayList<>();
    for (LzsTradeManagementSnapshot s : items) {
      if (s == null) continue;
      if (lookbackMs <= 0L || nowMs - s.nowMs <= lookbackMs) recent.add(s);
    }
    if (recent.size() < 2) return Double.NaN;
    int lastCompleted = recent.size() - 2;
    if (lastCompleted < 0) return Double.NaN;
    if (side == LzsSide.LONG) {
      for (int i = lastCompleted - 1; i >= 1; i--) {
        double prev = recent.get(i - 1).low;
        double cur = recent.get(i).low;
        double next = recent.get(i + 1).low;
        if (!Double.isNaN(cur) && !Double.isNaN(prev) && !Double.isNaN(next) && cur <= prev && cur <= next) return cur;
      }
      return recent.get(lastCompleted).low;
    }
    for (int i = lastCompleted - 1; i >= 1; i--) {
      double prev = recent.get(i - 1).high;
      double cur = recent.get(i).high;
      double next = recent.get(i + 1).high;
      if (!Double.isNaN(cur) && !Double.isNaN(prev) && !Double.isNaN(next) && cur >= prev && cur >= next) return cur;
    }
    return recent.get(lastCompleted).high;
  }
