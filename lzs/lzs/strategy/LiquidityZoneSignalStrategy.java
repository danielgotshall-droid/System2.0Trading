...
import study_examples.lzs.strategy.management.LzsTradeManagementAction;
import study_examples.lzs.strategy.management.LzsTradeManagementDecision;
...
import study_examples.lzs.strategy.management.LzsTradeManagementPolicy;
...
public class LiquidityZoneSignalStrategy extends LiquidityZoneSignalStudy {
  ...
  private static final String STRAT_MGMT_ACTIONS_ENABLED = "stratMgmtActionsEnabled";
  private static final String STRAT_PROTECT_ACTION_ENABLED = "stratProtectActionEnabled";
  private static final String STRAT_PROTECT_ARM_MFE_TICKS = "stratProtectArmMfeTicks";
  private static final String STRAT_PROTECT_PERSIST_MS = "stratProtectPersistMs";
  private static final String STRAT_PROTECT_BE_CUSHION_TICKS = "stratProtectBeCushionTicks";
  private static final String STRAT_STRUCT_PROTECT_ENABLED = "stratStructProtectEnabled";
  private static final String STRAT_STRUCT_PROTECT_LOOKBACK_MS = "stratStructProtectLookbackMs";
  private static final String STRAT_STRUCT_PROTECT_BUFFER_TICKS = "stratStructProtectBufferTicks";
  private static final String STRAT_STRUCT_PROTECT_MIN_DRR = "stratStructProtectMinDrr";
  private static final String STRAT_STRUCT_PROTECT_REQUIRE_BAI_OK = "stratStructProtectRequireBaiOk";
  private static final String STRAT_EXIT_RISK_ACTION_ENABLED = "stratExitRiskActionEnabled";
  private static final String STRAT_EXIT_RISK_ACTION_MODE = "stratExitRiskActionMode";
  private static final String STRAT_EXIT_RISK_PERSIST_MS = "stratExitRiskPersistMs";
  private static final String STRAT_EXIT_RISK_TIGHTEN_BUFFER_TICKS = "stratExitRiskTightenBufferTicks";
  private static final String STRAT_EXIT_RISK_USE_STRUCTURAL_REF = "stratExitRiskUseStructuralRef";
  private static final String STRAT_FORCE_EXIT_ACTION_ENABLED = "stratForceExitActionEnabled";
  private static final String STRAT_MIN_HOLD_ENABLED = "stratMinHoldEnabled";
  private static final String STRAT_MIN_HOLD_SECONDS = "stratMinHoldSeconds";
  private static final String STRAT_MIN_HOLD_ALLOW_FORCE_EXIT = "stratMinHoldAllowForceExit";
  private static final String STRAT_MIN_HOLD_ALLOW_INITIAL_STOP = "stratMinHoldAllowInitialStop";
  ...
  private final LzsTradeManagementPolicy managementPolicy;
  private String lastStatusLine = "STRAT FLAT";

  private LzsTradeManagementState lastMgmtStateSeen = LzsTradeManagementState.MGMT_INACTIVE;
  private long mgmtStateSinceMs = 0L;
  private String lastMgmtAction = "";
  private String lastMgmtActionReason = "";
  ...
  public LiquidityZoneSignalStrategy(LzsStrategyConfig config) {
    ...
    syncManagementActionConfig();
    this.management = new LzsTradeManagementEngine(this.config.management);
    this.managementPolicy = new LzsTradeManagementPolicy(this.config.management);
  }
  ...
  public void initialize(Defaults defaults) {
    ...
    if (sd != null) {
      ...
      SettingGroup mgmtActions = stratHudTab.addGroup("Management Actions");
      mgmtActions.addRow(new BooleanDescriptor(STRAT_MGMT_ACTIONS_ENABLED, "Enable Management Actions", config.managementActionsEnabled));
      mgmtActions.addRow(new BooleanDescriptor(STRAT_PROTECT_ACTION_ENABLED, "Enable Protect Stop Tightening", config.protectActionEnabled));
      mgmtActions.addRow(new DoubleDescriptor(STRAT_PROTECT_ARM_MFE_TICKS, "Protect Arm MFE (ticks)", config.protectArmMfeTicks, 0.0, 50.0, 0.25));
      mgmtActions.addRow(new IntegerDescriptor(STRAT_PROTECT_PERSIST_MS, "Protect Persistence (ms)", (int) config.protectPersistenceMs, 0, 20000, 100));
      mgmtActions.addRow(new DoubleDescriptor(STRAT_PROTECT_BE_CUSHION_TICKS, "Break-even Cushion (ticks)", config.protectBreakevenCushionTicks, 0.0, 10.0, 0.25));
      mgmtActions.addRow(new BooleanDescriptor(STRAT_STRUCT_PROTECT_ENABLED, "Enable Structural Protect Trailing", config.structuralProtectEnabled));
      mgmtActions.addRow(new IntegerDescriptor(STRAT_STRUCT_PROTECT_LOOKBACK_MS, "Structural Protect Lookback (ms)", (int) config.structuralProtectLookbackMs, 0, 20000, 100));
      mgmtActions.addRow(new DoubleDescriptor(STRAT_STRUCT_PROTECT_BUFFER_TICKS, "Structural Protect Buffer (ticks)", config.structuralProtectBufferTicks, 0.0, 10.0, 0.25));
      mgmtActions.addRow(new DoubleDescriptor(STRAT_STRUCT_PROTECT_MIN_DRR, "Structural Protect Min DRR", config.structuralProtectMinDrr, 0.0, 1.0, 0.05));
      mgmtActions.addRow(new BooleanDescriptor(STRAT_STRUCT_PROTECT_REQUIRE_BAI_OK, "Require BAI OK For Structural Protect", config.structuralProtectRequireBaiOk));
      mgmtActions.addRow(new BooleanDescriptor(STRAT_EXIT_RISK_ACTION_ENABLED, "Enable Exit Risk Actions", config.exitRiskActionEnabled));
      java.util.List<NVP> exitRiskModes = java.util.Arrays.asList(
          new NVP("Tighten Only", "0"),
          new NVP("Exit Market", "1"));
      mgmtActions.addRow(new DiscreteDescriptor(STRAT_EXIT_RISK_ACTION_MODE, "Exit Risk Action Mode", String.valueOf(config.exitRiskActionMode), exitRiskModes));
      mgmtActions.addRow(new IntegerDescriptor(STRAT_EXIT_RISK_PERSIST_MS, "Exit Risk Persistence (ms)", (int) config.exitRiskPersistenceMs, 0, 20000, 100));
      mgmtActions.addRow(new DoubleDescriptor(STRAT_EXIT_RISK_TIGHTEN_BUFFER_TICKS, "Exit Risk Tighten Buffer (ticks)", config.exitRiskTightenBufferTicks, 0.0, 20.0, 0.25));
      mgmtActions.addRow(new BooleanDescriptor(STRAT_EXIT_RISK_USE_STRUCTURAL_REF, "Exit Risk Use Structural Ref", config.exitRiskUseStructuralRef));
      mgmtActions.addRow(new BooleanDescriptor(STRAT_FORCE_EXIT_ACTION_ENABLED, "Enable Force Exit Action", config.forceExitActionEnabled));
      SettingGroup minHold = stratHudTab.addGroup("Minimum Hold Time");
      minHold.addRow(new BooleanDescriptor(STRAT_MIN_HOLD_ENABLED, "Enable Minimum Hold Time", config.minimumHoldEnabled));
      minHold.addRow(new DoubleDescriptor(STRAT_MIN_HOLD_SECONDS, "Minimum Hold Time (seconds)", config.minimumHoldSeconds, 0.0, 120.0, 0.5));
      minHold.addRow(new BooleanDescriptor(STRAT_MIN_HOLD_ALLOW_FORCE_EXIT, "Allow Force Exit To Override Hold", config.allowForceExitDuringMinimumHold));
      minHold.addRow(new BooleanDescriptor(STRAT_MIN_HOLD_ALLOW_INITIAL_STOP, "Allow Initial Stop To Override Hold", config.allowInitialStopDuringMinimumHold));
    }
    ...
    rd.exportValue(new ValueDescriptor(LzsStrategyValues.ACTIVE_STOP, "STRAT Active Stop"));
    ...
    rd.exportValue(new ValueDescriptor(LzsStrategyValues.MGMT_ACTION, "STRAT MGMT Action"));
  ...
  public String buildStatusLine() {
    ...
    long holdRemainingMs = minimumHoldRemainingMs(System.currentTimeMillis());
    if (holdRemainingMs > 0L) sb.append(" | MINHOLD ").append(fmt(holdRemainingMs / 1000.0)).append('s');
  ...
  private void refreshManagement(OrderContext ctx) {
    ...
    LzsTradeManagementSnapshot snapshot = buildManagementSnapshot(series, index, tickSize, nowMs);
    LzsTradeManagementScores scores = management.update(snapshot);
    updateManagementStateClock(scores == null ? null : scores.state, nowMs);
    applyManagementAction(ctx, scores, management.getLastMetrics(), tickSize, nowMs);
    writeStrategyValues(ctx);
  ...
  private void writeStrategyValues(OrderContext ctx) {
    ...
    series.setDouble(index, LzsStrategyValues.INITIAL_STOP, position.initialStopPrice);
    series.setDouble(index, LzsStrategyValues.ACTIVE_STOP, position.stopPrice);
    ...
    series.setDouble(index, LzsStrategyValues.MGMT_ACTION,
        "TIGHTEN_STOP".equals(lastMgmtAction) ? 1.0 :
        ("KEEP_STOP".equals(lastMgmtAction) ? 2.0 :
        ("EXIT_MARKET".equals(lastMgmtAction) ? 3.0 : Double.NaN)));
    ...
    series.setDouble(index, LzsStrategyValues.LAST_ORDER_ACTION,
        ...
        ("MGMT_EXIT_SENT".equals(lastOrderAction) ? 5.0 :
        ("ORDER_ERROR".equals(lastOrderAction) ? -2.0 :
        ("STOP_EXIT_ERROR".equals(lastOrderAction) ? -4.0 :
        ("MGMT_EXIT_ERROR".equals(lastOrderAction) ? -5.0 : Double.NaN)))))))));
  ...
  private void updateManagementStateClock(LzsTradeManagementState state, long nowMs) {
    LzsTradeManagementState s = state == null ? LzsTradeManagementState.MGMT_INACTIVE : state;
    if (s != lastMgmtStateSeen) {
      lastMgmtStateSeen = s;
      mgmtStateSinceMs = nowMs;
    }
  }

  private void resetManagementActionState() {
    lastMgmtStateSeen = LzsTradeManagementState.MGMT_INACTIVE;
    mgmtStateSinceMs = 0L;
    lastMgmtAction = "";
    lastMgmtActionReason = "";
  }

  private void applyManagementAction(OrderContext ctx, LzsTradeManagementScores scores, LzsTradeManagementMetrics metrics, double tickSize, long nowMs) {
    if (position.isFlat() || !position.isActive()) {
      lastMgmtAction = "";
      lastMgmtActionReason = "";
      return;
    }
    double structuralRef = management.getStructuralProtectReference(position.side, config.management.structuralProtectLookbackMs);
    LzsTradeManagementDecision d = managementPolicy.decide(scores, metrics, structuralRef, position, tickSize, nowMs, mgmtStateSinceMs);
    if (d == null) {
      lastMgmtAction = "";
      lastMgmtActionReason = "";
      return;
    }
    lastMgmtAction = d.action == null ? "" : d.action.name();
    lastMgmtActionReason = d.reason == null ? "" : d.reason;
    long holdRemainingMs = minimumHoldRemainingMs(nowMs);
    if (holdRemainingMs > 0L && d.action != null && d.action != LzsTradeManagementAction.NONE
        && d.action != LzsTradeManagementAction.KEEP_STOP) {
      boolean allowOverride = d.action == LzsTradeManagementAction.EXIT_MARKET
          && scores != null
          && scores.state == LzsTradeManagementState.MGMT_FORCE_EXIT
          && allowForceExitDuringMinimumHold();
      if (!allowOverride) {
        lastMgmtAction = "HOLD_GATE";
        lastMgmtActionReason = "minHold " + fmt(holdRemainingMs / 1000.0) + "s";
        return;
      }
    }
    if (d.action == LzsTradeManagementAction.TIGHTEN_STOP && !Double.isNaN(d.targetStopPrice)) {
      String mode = scores != null && scores.state == LzsTradeManagementState.MGMT_EXIT_RISK ? "EXITRISK" : ("protectStruct".equals(d.reason) ? "STRUCT" : "PROTECT");
      if (position.tightenStop(d.targetStopPrice, mode)) {
        lastMgmtActionReason = (lastMgmtActionReason.isEmpty() ? "" : lastMgmtActionReason + " ") + "->" + fmt(d.targetStopPrice);
      }
      else {
        lastMgmtAction = "KEEP_STOP";
        lastMgmtActionReason = "noTighten";
      }
      return;
    }
    if (d.action == LzsTradeManagementAction.EXIT_MARKET) {
      if (position.lifecycle == LzsPositionLifecycle.EXIT_PENDING) {
        lastMgmtActionReason = "alreadyExitPending";
        return;
      }
      position.markExitPending();
      lastOrderAction = "MGMT_EXIT_SENT";
      ...
      try {
        ...
        lastOrderAction = "MGMT_EXIT_ERROR";
        lastOrderError = t == null ? "unknown" : (t.getClass().getSimpleName() + ": " + String.valueOf(t.getMessage()));
      }
    }
  }
  ...
  private boolean enforceInitialStop(OrderContext ctx) {
    ...
    if (!hit) return false;
    long nowMs = System.currentTimeMillis();
    if (isMinimumHoldActive(nowMs) && !allowInitialStopDuringMinimumHold()) {
      lastOrderAction = "HOLD_GATE";
      lastOrderError = "MinHold initial stop blocked";
      updateRuntimeState("minHoldInitStop");
      writeStrategyValues(ctx);
      return false;
    }
  ...
  private String buildStrategyHudText() {
    ...
    sb.append("POS ").append(position.side).append(' ').append(position.lifecycle)
      ...
      .append(" | EP ").append(fmt(position.entryPrice))
      .append(" | STP ").append(fmt(position.stopPrice));
    if (!Double.isNaN(position.initialStopPrice)) sb.append(" | ISTP ").append(fmt(position.initialStopPrice));
    if (position.stopMode != null && !position.stopMode.isEmpty()) sb.append(" | SM ").append(position.stopMode);
    ...
    if (scores == null || scores.state == null) {
      ...
      sb.append("MGMT ").append(shortState(scores.state))
        ...
        .append(" | ERS ").append(fmt(scores.ers));
      if (scores.reasons != null && !scores.reasons.isEmpty()) sb.append(" | ").append(scores.reasons);
      if (lastMgmtAction != null && !lastMgmtAction.isEmpty()) sb.append(" | ACT ").append(lastMgmtAction);
      if (lastMgmtActionReason != null && !lastMgmtActionReason.isEmpty()) sb.append(" (").append(lastMgmtActionReason).append(")");
    }
  ...
  private void syncManagementActionConfig() {
    if (getSettings() == null) {
      config.management.actionsEnabled = config.managementActionsEnabled;
      config.management.protectActionEnabled = config.protectActionEnabled;
      config.management.protectArmMfeTicks = config.protectArmMfeTicks;
      config.management.protectPersistenceMs = config.protectPersistenceMs;
      config.management.protectBreakevenCushionTicks = config.protectBreakevenCushionTicks;
      config.management.structuralProtectEnabled = config.structuralProtectEnabled;
      config.management.structuralProtectLookbackMs = config.structuralProtectLookbackMs;
      config.management.structuralProtectBufferTicks = config.structuralProtectBufferTicks;
      config.management.structuralProtectMinDrr = config.structuralProtectMinDrr;
      config.management.structuralProtectRequireBaiOk = config.structuralProtectRequireBaiOk;
      config.management.exitRiskActionEnabled = config.exitRiskActionEnabled;
      config.management.exitRiskActionMode = config.exitRiskActionMode;
      config.management.exitRiskPersistenceMs = config.exitRiskPersistenceMs;
      config.management.exitRiskTightenBufferTicks = config.exitRiskTightenBufferTicks;
      config.management.exitRiskUseStructuralRef = config.exitRiskUseStructuralRef;
      config.management.forceExitActionEnabled = config.forceExitActionEnabled;
      config.minimumHoldEnabled = config.minimumHoldEnabled;
      config.minimumHoldSeconds = config.minimumHoldSeconds;
      config.allowForceExitDuringMinimumHold = config.allowForceExitDuringMinimumHold;
      config.allowInitialStopDuringMinimumHold = config.allowInitialStopDuringMinimumHold;
      return;
    }
    config.management.actionsEnabled = getSettings().getBoolean(STRAT_MGMT_ACTIONS_ENABLED, config.managementActionsEnabled);
    config.management.protectActionEnabled = getSettings().getBoolean(STRAT_PROTECT_ACTION_ENABLED, config.protectActionEnabled);
    config.management.protectArmMfeTicks = getSettings().getDouble(STRAT_PROTECT_ARM_MFE_TICKS, config.protectArmMfeTicks);
    config.management.protectPersistenceMs = getSettings().getInteger(STRAT_PROTECT_PERSIST_MS, (int) config.protectPersistenceMs);
    config.management.protectBreakevenCushionTicks = getSettings().getDouble(STRAT_PROTECT_BE_CUSHION_TICKS, config.protectBreakevenCushionTicks);
    config.management.structuralProtectEnabled = getSettings().getBoolean(STRAT_STRUCT_PROTECT_ENABLED, config.structuralProtectEnabled);
    config.management.structuralProtectLookbackMs = getSettings().getInteger(STRAT_STRUCT_PROTECT_LOOKBACK_MS, (int) config.structuralProtectLookbackMs);
    config.management.structuralProtectBufferTicks = getSettings().getDouble(STRAT_STRUCT_PROTECT_BUFFER_TICKS, config.structuralProtectBufferTicks);
    config.management.structuralProtectMinDrr = getSettings().getDouble(STRAT_STRUCT_PROTECT_MIN_DRR, config.structuralProtectMinDrr);
    config.management.structuralProtectRequireBaiOk = getSettings().getBoolean(STRAT_STRUCT_PROTECT_REQUIRE_BAI_OK, config.structuralProtectRequireBaiOk);
    config.management.exitRiskActionEnabled = getSettings().getBoolean(STRAT_EXIT_RISK_ACTION_ENABLED, config.exitRiskActionEnabled);
    config.management.exitRiskActionMode = parseExitRiskActionMode(getSettings().getString(STRAT_EXIT_RISK_ACTION_MODE, String.valueOf(config.exitRiskActionMode)));
    config.management.exitRiskPersistenceMs = getSettings().getInteger(STRAT_EXIT_RISK_PERSIST_MS, (int) config.exitRiskPersistenceMs);
    config.management.exitRiskTightenBufferTicks = getSettings().getDouble(STRAT_EXIT_RISK_TIGHTEN_BUFFER_TICKS, config.exitRiskTightenBufferTicks);
    config.management.exitRiskUseStructuralRef = getSettings().getBoolean(STRAT_EXIT_RISK_USE_STRUCTURAL_REF, config.exitRiskUseStructuralRef);
    config.management.forceExitActionEnabled = getSettings().getBoolean(STRAT_FORCE_EXIT_ACTION_ENABLED, config.forceExitActionEnabled);
    config.minimumHoldEnabled = getSettings().getBoolean(STRAT_MIN_HOLD_ENABLED, config.minimumHoldEnabled);
    config.minimumHoldSeconds = getSettings().getDouble(STRAT_MIN_HOLD_SECONDS, config.minimumHoldSeconds);
    config.allowForceExitDuringMinimumHold = getSettings().getBoolean(STRAT_MIN_HOLD_ALLOW_FORCE_EXIT, config.allowForceExitDuringMinimumHold);
    config.allowInitialStopDuringMinimumHold = getSettings().getBoolean(STRAT_MIN_HOLD_ALLOW_INITIAL_STOP, config.allowInitialStopDuringMinimumHold);
  }

  private int parseExitRiskActionMode(String v) {
    if (v == null) return config.exitRiskActionMode;
    try { return Integer.parseInt(v.trim()); }
    catch (Throwable ignored) { return config.exitRiskActionMode; }
  }

  private long minimumHoldRemainingMs(long nowMs) {
    if (!config.minimumHoldEnabled || position == null || !position.isActive() || position.entryTime <= 0L) return 0L;
    long minMs = Math.max(0L, Math.round(config.minimumHoldSeconds * 1000.0));
    if (minMs <= 0L) return 0L;
    long until = position.entryTime + minMs;
    return Math.max(0L, until - nowMs);
  }

  private boolean isMinimumHoldActive(long nowMs) {
    return minimumHoldRemainingMs(nowMs) > 0L;
  }

  private boolean allowForceExitDuringMinimumHold() {
    return config.allowForceExitDuringMinimumHold;
  }

  private boolean allowInitialStopDuringMinimumHold() {
    return config.allowInitialStopDuringMinimumHold;
  }
